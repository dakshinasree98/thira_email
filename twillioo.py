...
def twilio_webhook():
    ...
    if query_type == "1":
        # Ask for email ID
        response_text = "Please provide your email ID so we can send you the booking details."
    elif query_type == "2":
...
# Twilio webhook for handling email ID and sending booking details
@app.route('/twilio_email_webhook', methods=['POST'])
def twilio_email_webhook():
    phone_number = request.form.get('From')
    email_id = request.form.get('Body')

    if not phone_number or not email_id:
        error_message = "<Response><Message>Error: Phone number and email ID are required.</Message></Response>"
        return error_message, 400, {'Content-Type': 'application/xml'}

    logger.info(f"Received email ID from {phone_number}: {email_id}")

    # Fetch room details
    room_details = fetch_room_details()

    # Send email with booking details
    email_subject = "Your Booking Details at Thira Beach Home"
    email_body = f"Thank you for choosing Thira Beach Home. Here are your booking details:\n\n{room_details}"
    email_sent = send_email(email_id, email_subject, email_body)

    if email_sent:
        response_text = f"An email with your booking details has been sent to {email_id}."
    else:
        response_text = "Sorry, we encountered an issue while sending the email. Please try again later."
